...

{
   ...
   const run = (em.run = async (oResult, mode = "normal") => {
      ...
      try {
            ...
            await Promise.all([
               ocr("eng", src).then((o) => {
                  return new Promise((resolve) =>
                     chrome.i18n.detectLanguage(o.text, (r) => {
                        resolve({
                           lang: "eng",
                           o,
                           r,
               ...
               if (r.r.languages.length) {
                  ...
                  command(
                     "message",
                     `Detected language is "${e || r.lang}". Please wait...`
                  );
               }
               o = o || r.o;
            });
         ...
         if (o.confidence < 50 && !oResult) {
            command(
               "message",
               `Low confidence (${o.confidence}%). Trying with inverted colors. Please wait...`
            );
         ...
         if (
            mode === "invert" &&
            o.confidence < 20 &&
            oResult.confidence < 20
         ) {
            command(
               "message",
               `Low confidence (${o.confidence}%) again! Trying with inverted colors. Please wait...`
            );
         ...
         chrome.runtime.sendMessage(
            ...
            (granted) => {
               if (granted === undefined) {
                  alert(
                     `Please enable the "Input data to the clipboard" permission for this extension in the browser's extensions manager.`
                  );
               }
            }
         );
   ...
   em.addEventListener("drop", async (e) => {
      e.preventDefault();
      for (const entry of [...e.dataTransfer.files].filter(
